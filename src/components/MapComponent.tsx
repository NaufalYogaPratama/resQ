...
export default function MapComponent({ userId, userRole }: MapComponentProps) {
  ...
  return (
    <div className="relative h-full">
      {/* Filter kategori dengan tema terang */}
          ...
          >
            <Popup>
              <div className="w-64">
                ...
                {report.gambarUrl ? (
                  <img src={report.gambarUrl} alt="Kejadian" className="w-full h-32 object-cover rounded-lg my-2"/>
                ) : (
                  <div className="w-full h-32 bg-slate-100 flex items-center justify-center rounded-lg my-2">
                    <span className="text-slate-400 text-sm">Tidak ada foto</span>
                  </div>
                )}

                <p className="text-slate-700 text-sm">{report.deskripsi}</p>
                {report.lokasi.alamat && (
                  <p className="text-xs text-slate-500 italic mt-1">Lokasi: {report.lokasi.alamat}</p>
                )}
                <p className="text-xs text-slate-500 mt-2 border-t pt-2">Pelapor: {report.pelapor.namaLengkap}</p>

                {/* Tombol Aksi (Relawan & Pelapor) */}
                {userRole === 'Relawan' && report.status === 'Menunggu' && (
                  <button
                    onClick={() => handleClaim(report._id)}
                    disabled={isSubmitting === report._id}
                    className="w-full mt-2 bg-indigo-600 text-white text-sm font-semibold py-2 px-4 rounded-lg hover:bg-indigo-500 disabled:bg-slate-400"
                  >
      ...
      {/* Styling untuk pin. Popup akan menggunakan style default Leaflet yang terang. */}
      <style jsx global>{`
        .marker-pin {
          width: 25px;
          height: 25px;
