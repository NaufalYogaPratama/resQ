...
export default function WaraChatbot({ onComplete, onClose }: WaraChatbotProps) {
  const [messages, setMessages] = useState<Message[]>([
    { sender: 'bot', text: 'Halo! Saya WARA, asisten darurat Anda. Ceritakan kejadian yang Anda lihat dalam satu kalimat.' }
  ]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorResponse: Message = { sender: 'bot', text: `Maaf, terjadi kesalahan: ${err.message}` };
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg flex flex-col h-[70vh]">
        <div className="p-4 flex justify-between items-center border-b bg-slate-50 rounded-t-2xl">
          <div className="flex items-center gap-3">
            <div className="bg-indigo-600 p-2 rounded-full"><Bot className="w-6 h-6 text-white" /></div>
            <div>
          ...
          <button onClick={onClose} className="text-slate-500 hover:text-slate-800"><X className="w-6 h-6" /></button>
        </div>

        <div className="flex-grow p-4 overflow-y-auto space-y-4">
          {messages.map((msg, index) => (
            <div key={index} className={`flex items-start gap-3 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
              {msg.sender === 'bot' && <div className="bg-slate-200 p-2 rounded-full"><Bot className="w-5 h-5 text-slate-600"/></div>}
              ...
              {msg.sender === 'user' && <div className="bg-slate-200 p-2 rounded-full"><User className="w-5 h-5 text-slate-600"/></div>}
            </div>
          ))}
          {isLoading && (
            <div className="flex items-start gap-3">
               <div className="bg-slate-200 p-2 rounded-full"><Bot className="w-5 h-5 text-slate-600"/></div>
        ...
        <div className="p-4 border-t">
          <form onSubmit={handleSendMessage} className="flex items-center gap-2">
            <input
              ...
              disabled={isLoading}
            />
            <button type="submit" className="bg-indigo-600 text-white p-3 rounded-lg hover:bg-indigo-700 disabled:bg-slate-400 flex-shrink-0">
