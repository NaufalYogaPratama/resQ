  ...
  return (
    <div className="bg-slate-50 min-h-screen p-4 sm:p-8 font-sans">
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          <Link href="/relawan/laporan" className="inline-flex items-center gap-2 text-slate-600 hover:text-green-600 font-semibold">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="p-8 space-y-6">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                ...
                {/* Tombol Aksi untuk Relawan akan ditambahkan di sini nanti */}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t">
              <div>
                ...
                <ul className="space-y-2 text-slate-600">
                  <li className="flex items-center gap-3"><Tag className="w-5 h-5 text-green-500"/> <strong>Kategori:</strong> {report.kategori}</li>
                  <li className="flex items-start gap-3"><MapPin className="w-5 h-5 text-green-500 mt-1"/> <strong>Alamat:</strong> {report.lokasi.alamat || 'Tidak ada alamat'}</li>
                  <li className="flex items-center gap-3"><Calendar className="w-5 h-5 text-green-500"/> <strong>Waktu Lapor:</strong> {new Date(report.createdAt).toLocaleString('id-ID')}</li>
                </ul>
              </div>
              <div>
                ...
                <ul className="space-y-2 text-slate-600">
                  <li className="flex items-center gap-3">
                   ...
                   <li className="flex items-center gap-3">
                     <a href={`https://wa.me/${report.pelapor.noWa.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-green-600 font-semibold hover:underline">
                        <Phone className="w-4 h-4"/> Hubungi Pelapor
                      </a>
                  </li>
                  {report.penolong && (
                     <li className="flex items-center gap-3"><Shield className="w-5 h-5 text-green-500"/> <strong>Ditangani oleh:</strong> {report.penolong.namaLengkap}</li>
                  )}
                </ul>
              </div>
            </div>
